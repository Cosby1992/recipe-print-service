...
export const recipeRouter = router.post<{}, RecipesResponseDto, RecipeRequestDto>(
  ...
  expressAsyncHandler(async (req, res, _next) => {
    ...
    if (!recipesResponseDto) {
      ...
      throw new InternalServerError(['Failed to get or clean response from ChatGPT.']);
    }
    ...
    const scaledRecipes = recipesResponseDto.recipes.map((recipe) => scaleRecipe(recipe, targetPortions));
